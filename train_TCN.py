...
def get_arguments():
    ...
    parser.add_argument("--weight_decay", type=float, default=0) # 新たに追加
...
class AugmentedDataset(torch.utils.data.Dataset):
    def __init__(self, data, labels, transform=False):
        self.data = data
        self.labels = labels
        self.transform = transform # Trueならデータ拡張を行う

    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx):
        # データを取得（コピーを作成して元のデータを書き換えないようにする）
        ...
        if self.transform:
            ...
            # 1. タイムシフト (Time Shift)
            # 波形を左右にランダムにずらす（±20ステップ）
            shift = np.random.randint(-20, 20)
            x = np.roll(x, shift, axis=-1)
            
            # 2. 振幅スケーリング (Amplitude Scaling)
            # 波形の大きさを0.9倍〜1.1倍の範囲でランダムに変える
            scale = np.random.uniform(0.9, 1.1)
            x = x * scale
            
            # 3. ノイズ注入 (Gaussian Noise Injection)
            # 微量なノイズを足す（標準偏差0.05程度）
            noise = np.random.normal(0, 0.05, x.shape)
            x = x + noise

        # PyTorchのTensorに変換 (float32型)
...
def train_model(net, dataloaders_dict, criterion, \
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") # デバイス取得を追加
    train_loss, train_acc, val_loss, val_acc = [], [], [], []
    max_acc = 0

    # ▼▼▼ 1. Early Stopping用の変数を初期化 ▼▼▼
    patience = 20          # 20エポック更新がなければ終了
    no_improve_count = 0   # 更新がない期間をカウントする変数
    # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
    ...
    for epoch in range(num_epochs):
        ...
        # for phase ループの外（1エポックの終わり）でチェック
        
        # ▼▼▼ 3. 強制終了のチェック ▼▼▼
        if no_improve_count >= patience:
            print(f'\nEarly Stopping: {patience}エポック連続で精度向上がなかったため終了します。')
    ...
    for n_sectors in n_sectors_list:
        ...
        if args.method==1:
            ...
            net = TCN(
                ...
                num_channels=[64]*7, kernel_size=7, dropout=args.dropout
            )# num_cahnnelsを64→32→64に変更,kernel_sizeを3→5→7に変更
             # 128の場合の方が良いかも？
        ...
        # ▼▼▼ ここを書き換え ▼▼▼
        # importしたcreate_train_dataloaderではなく、
        # 新しく作った create_augmented_dataloader を使う
